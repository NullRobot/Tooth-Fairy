...
# Function to display the contents of robots.txt if it exists and extract cookies
display_robots_txt() {
    local domain="$1"
    local robots_txt_url="http://${domain}/robots.txt"
    wget -q -O robots.txt "$robots_txt_url"
    if [ -s robots.txt ]; then
        print_table_header "Contents of robots.txt"
        cat robots.txt
    else
        print_table_header "No robots.txt found or it is empty"
    fi
    rm -f robots.txt # Clean up the downloaded file

    # Extract cookies from HTTP header
    local cookies=$(wget --save-headers -q -S --spider "$robots_txt_url" 2>&1 | grep -o 'Set-Cookie: .*' | sed 's/Set-Cookie: //' || echo "NONE!")
...
extract_web_info() {
    ...
    sleep 1
    # Title
    local title=$(grep -o '<title>[^<]*</title>' "$file" | sed 's/<title>\(.*\)<\/title>/\1/' || echo "NONE!")
    print_table_header "TITLE:"
    echo "$title"
    # HTML Comments
    local comments=$(grep -o '<!--.*?-->' "$file" || echo "NONE!")
    print_table_header "HTML COMMENTS:"
    echo "$comments"
    # Links
    local links=$(grep -o 'href="[^"]*"' "$file" | sed 's/href="//;s/"$//' || echo "NONE!")
    print_table_header "LINKS:"
    echo "$links"
    # Forms
    local forms=$(sed -n '/<form/,/<\/form>/p' "$file" || echo "NONE!")
    print_table_header "FORMS:"
    echo "$forms"
}
...
if [[ "$INPUT" =~ ^https?:// ]]; then
    ...
    wget --no-check-certificate -mk -nv --no-use-server-timestamps -A html,css,js,php,txt,csv,json,xml,htm,asp,aspx,jsp,cgi,pl,md,yml,yaml,ini,ts,jsx,tsx,scss,less,sass -R jpg,jpeg,png,gif,svg,ico,mp3,mp4,avi,mov,mkv,flv,wmv,webm,ogg,wav,m4a,flac,mpg,mpeg,iso,img,bin,exe,dmg "$INPUT" 
    # Assuming wget creates a directory with the domain name
    DOMAIN=$(echo "$INPUT" | awk -F/ '{print $3}')
    if [ -d "$DOMAIN" ]; then
	display_robots_txt "$DOMAIN" # Call the function to display robots.txt
...
case "$FILE_TYPE" in
    text/html|application/x-php|text/plain|text/csv|text/javascript|text/css)
